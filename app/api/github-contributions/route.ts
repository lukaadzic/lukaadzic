...
function generateFallbackContributions(): { contributions: Array<{ date: string; count: number; level: number }>, totalContributions: number } {
  ...
  for (let d = new Date(yearAgo); d <= today; d.setDate(d.getDate() + 1)) {
    ...
    const weekOfYear = Math.floor((d.getTime() - yearAgo.getTime()) / (7 * 24 * 60 * 60 * 1000));
